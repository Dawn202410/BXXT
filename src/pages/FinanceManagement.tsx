...
const isValidRecord = (record: any): boolean => {
  return (
    ...
    (record.status === 'paid' || record.status === 'pending' || record.status === 'refunded')
  );
};
...
export default function FinanceManagement() {
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    ...
                    setRecords(updatedFinanceRecords.filter(isValidRecord));
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={timeData()}>
                ...
                <YAxis />
                <Tooltip formatter={(value) => [`¥${value}`, '金额']} />
                <Area type="monotone" dataKey="income" stroke="#4A90E2" fill="#4A90E2" fillOpacity={0.2} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
  ...
  const handleStatusChange = async (recordId: string, newStatus: string) => {
    try {
      // 验证状态值
      const validStatuses = ['paid', 'pending', 'refunded', 'pending_invoice', 'pending_payment'];
      ...
      const financeRecords = JSON.parse(localStorage.getItem('financeRecords') || '[]');
      const repairs = JSON.parse(localStorage.getItem('repairs') || '[]');
      if (!financeRecords || !repairs) {
        throw new Error('未找到财务记录数据');
      }
      
      // 严格验证记录是否存在
      const recordIndex = financeRecords.findIndex((r: any) => r.id === recordId);
      if (recordIndex === -1) {
        throw new Error('未找到要更新的记录，可能已被删除');
      }

      const recordToUpdate = financeRecords[recordIndex];
      if (!recordToUpdate || typeof recordToUpdate !== 'object') {
        throw new Error('找到的记录数据无效');
      }

      // 验证支付方式与状态的兼容性
      const paymentMethod = recordToUpdate.paymentMethod || '现金';
      if (paymentMethod === '转账') {
        if (['pending', 'refunded'].includes(newStatus)) {
          throw new Error('对公转账记录不能设置为"待支付"或"已退款"状态');
        }
        // 对公转账只能从"待开票"到"已开票待支付"再到"已支付"
        if (recordToUpdate.status === 'pending_invoice' && newStatus !== 'pending_payment') {
          throw new Error('对公转账记录必须先从"待开票"变更为"已开票待支付"');
        }
        if (recordToUpdate.status === 'pending_payment' && newStatus !== 'paid') {
          throw new Error('对公转账记录必须从"已开票待支付"变更为"已支付"');
        }
      }

      // 验证关联工单是否存在（如果是服务记录）
      if (recordToUpdate.type === 'service' && recordToUpdate.repairId) {
        const repairExists = repairs.some((r: any) => r.id === recordToUpdate.repairId);
        if (!repairExists) {
          throw new Error('关联工单不存在，无法更新状态');
        }
      }
      ...
      // 更新记录数组
      const updatedFinanceRecords = [...financeRecords];
      updatedFinanceRecords[recordIndex] = updatedRecord;

      // 同步更新关联工单的支付状态（如果是服务记录）
      if (recordToUpdate.type === 'service' && recordToUpdate.repairId) {
        const updatedRepairs = repairs.map((repair: any) => {
          if (repair.id === recordToUpdate.repairId && repair.payment) {
            return {
              ...repair,
              payment: {
                ...repair.payment,
      ...
      toast.success(`记录状态已更新为: ${newStatus}`, {
        action: {
          label: '查看记录',
          onClick: () => {
      ...
      toast.error(`状态更新失败: ${errorMessage}`, {
        ...
        cancel: {
          ...
          onClick: () => {
            ...
            console.error('记录ID:', recordId);
            console.error('目标状态:', newStatus);
                        ...
                        <tr key={record.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                              {record.paymentMethod === '现金' ? '现金' : 
                               ...
                               record.paymentMethod === '支付宝支付' ? '支付宝支付' : '未指定'}
                          </td>
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {['现金', '微信支付', '支付宝支付'].includes(record.paymentMethod) ? (
                                  <span className="px-2 py-1 rounded-full text-xs bg-green-100 text-green-800 border">
                                    已支付
                                  </span>
                                ) : record.paymentMethod === '转账' ? (
                              <select
                                value={record.status}
                                onChange={(e) => {
                                  const statusNames = {
                                    ...
                                    'refunded': '已退款',
                                    'pending_invoice': '待开票',
                                    'pending_payment': '已开票待支付'
                                  };
                                  
                                  const confirmMessage = `确定要将记录状态从"${statusNames[record.status] || record.status}"变更为"${statusNames[e.target.value] || e.target.value}"吗？`;
                                ...
                                className={`px-2 py-1 rounded-full text-xs border ${
                                  ...
                                  record.status === 'pending_payment' ? 'bg-blue-100 text-blue-800' :
                                  record.status === 'pending_invoice' ? 'bg-purple-100 text-purple-800' :
                                ...
                                {record.paymentMethod === '转账' ? (
                                  record.status === 'pending_invoice' ? (
                                    <>
                                      ...
                                      <option value="pending_payment">已开票待支付</option>
                                    </>
